...
export default function MapComponent({ heritages, onPressHeritage, colorScheme }: MapComponentProps) {
  ...
  // Listener for clicking details inside the iframe
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      if (event.data && event.data.type === 'SELECT_HERITAGE') {
        ...
        markersData.forEach(marker => {
          const popupHtml = \`
            <div class="popup-content">
              ...
              <button class="popup-btn" onclick="window.parent.postMessage({type: 'SELECT_HERITAGE', id: '\${marker.id}'}, '*')">
                Xem chi tiết
              </button>
            </div>
          \`;
